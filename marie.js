 ...
 * @version 1.0.0
 * @license MIT
 * @description A simple JavaScript implementation of the MARIE architecture, a basic computer architecture used for educational purposes.
 * @author XHiddenProjects
 * @date 2025-21-04
 * @see {@link https://github.com/XHiddenProjects/Marie.js}
 */
class MarieJS{
    /**
    ...
    constructor(config={}){
        ...
        this.errors = []; // Initialize error logging array
    ...
    #load(addr){
        if(this.config.errorLogging && (addr < 0 || addr >= this.memory.length) || isNaN(addr)){
            ...
            this.errors.push(`Error (Load): Address ${addr} is out of bounds`);
    ...
    #store(addr){
        if(this.config.errorLogging && (addr < 0 || addr >= this.memory.length) || isNaN(addr)){
            // Log error if address is out of bounds
            this.errors.push(`Error (Store): Address ${addr} is out of bounds`);
            console.error(`Error: Address ${addr} is out of bounds`);
            return false;
        }
        this.MAR = addr;
        ...
        console.log(`Store: Memory[${this.MAR}] = ${this.AC}`);
    }
    /**
    ...
    #add(addr){
        if(this.config.errorLogging && (addr < 0 || addr >= this.memory.length) || isNaN(addr)){
            // Log error if address is out of bounds
            this.errors.push(`Error (Add): Address ${addr} is out of bounds`);
            console.error(`Error: Address ${addr} is out of bounds`);
            return false;
        }
        this.MAR = addr;
        ...
        console.log(`Add: AC = ${this.AC}`);
    }
    #subtract(addr){
        if(this.config.errorLogging && (addr < 0 || addr >= this.memory.length) || isNaN(addr)){
            // Log error if address is out of bounds
            this.errors.push(`Error (Subtract): Address ${addr} is out of bounds`);
            console.error(`Error: Address ${addr} is out of bounds`);
            return false;
        }
        this.MAR = addr;
    ...
    #fetch(){
        if(this.config.errorLogging && (this.PC < 0 || this.PC >= this.memory.length) || isNaN(this.PC)){
            this.errors.push(`Error (Fetch): Address ${this.PC} is out of bounds`);
    ...
    #exec(instruction){
        if(this.config.errorLogging && (instruction < 0 || instruction >= this.memory.length) || isNaN(instruction)){
            this.errors.push(`Error (Exec): Instruction ${instruction} is out of bounds`);
    ...
    #decode(instruction){
        if(this.config.errorLogging && (instruction < 0 || instruction >= this.memory.length) || isNaN(instruction)){
            // Log error if address is out of bounds
            this.errors.push(`Error (Decode): Instruction ${instruction} is out of bounds`);
            console.error(`Error: Instruction ${instruction} is out of bounds`);
            return;
        }
    ...
    run(){
        while(true){
            if(this.config.errorLogging && (this.PC < 0 || this.PC >= this.memory.length) || isNaN(this.PC)){
                // Log error if address is out of bounds
                this.errors.push(`Error (Run): Address ${this.PC} is out of bounds`);
                console.error(`Error: Address ${this.PC} is out of bounds`);
                break;
            }
            ...
            this.#update();
            if(instruction==this.ins.HALT()){
                console.log(`Program halted`);
                this.logging.push(`Program halted`);
                break;
            }
        }
    }
    ...
    assemble(programs){
        if(!Array.isArray(programs)){
            this.errors.push(`Error (Assemble): Programs should be an array`);
            throw new Error("Programs should be an array");
        }
        if(!programs.includes(this.ins.HALT())) programs.push(this.ins.HALT()); // Add halt instruction if not present
    ...
    getMemory(){
        if(this.config.errorLogging && (this.memory < 0 || this.memory >= this.memory.length) || isNaN(this.memory)){
            // Log error if address is out of bounds
            this.errors.push(`Error (Memory): Memory ${this.memory} is out of bounds`);
    ...
    /**
     * Returns the current state of the errors
     * @returns {String[]} Errors array
     */
    getErrors(){
        return this.errors.map((err)=>{return err.trim();});
    }
}